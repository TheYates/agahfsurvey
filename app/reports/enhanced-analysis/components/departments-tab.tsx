...
export function DepartmentsTab({
  ...
  useEffect(() => {
    const loadDepartmentFeedback = async () => {
      ...
      try {
        console.log("DepartmentsTab: Fetching department concerns...");
        const concerns = await fetchDepartmentConcerns();
        console.log("DepartmentsTab: Received department concerns:", concerns);
        setDepartmentConcerns(concerns);

        console.log("DepartmentsTab: Fetching recommendations...");
        const recommendations = await fetchRecommendations();
        console.log(
          "DepartmentsTab: Received recommendations:",
          recommendations
        );
  ...
  useEffect(() => {
    const loadAdditionalData = async () => {
      ...
      try {
        ...
        console.log("DepartmentsTab: Fetching visit time data...");
        const timeData = await fetchVisitTimeData();
        console.log("DepartmentsTab: Received visit time data:", timeData);
        setVisitTimeData(timeData);

        // Fetch patient type data for returning rates
        console.log("DepartmentsTab: Fetching patient type data...");
        const ptData = await fetchPatientTypeData();
        console.log("DepartmentsTab: Received patient type data:", ptData);
        setPatientTypeData(ptData);
      } catch (error) {
        console.error("Error fetching additional metrics data:", error);
      } finally {
  ...
  useEffect(() => {
    const generateSatisfactionTrend = async () => {
      try {
        console.log("DepartmentsTab: Fetching survey data for trends...");
        const surveyData = await fetchAllSurveyData();
        console.log("DepartmentsTab: Received survey data:", surveyData);
  ...
  if (isLoading || isLoadingAdditionalData) {
    return (
      <div className="flex flex-col items-center justify-center h-96">
        ...
        <p className="text-muted-foreground mt-4">Loading department data...</p>
      </div>
    );
  }
  ...
  if (selectedDepartment) {
    ...
    console.log("Selected department concerns:", departmentConcernsForSelected);
    ...
    let avgVisitTime = 0;
    console.log("DepartmentsTab: Visit time calculation data:", visitTimeData);
    if (visitTimeData.length > 0) {
      ...
      console.log("DepartmentsTab: Total visit time samples:", totalSamples);
      if (totalSamples > 0) {
        ...
        console.log(
          "DepartmentsTab: Calculated visit time from real data:",
          avgVisitTime
        );
      } else {
        avgVisitTime = 20; // Default if no data
        console.log(
          "DepartmentsTab: Using default visit time (no samples):",
          avgVisitTime
        );
      }
    } else {
      ...
      console.log(
        "DepartmentsTab: Using estimated visit time (no data):",
        avgVisitTime
      );
    }
    ...
    let returningRate = 0;
    console.log(
      "DepartmentsTab: Patient type data for returning rate:",
      patientTypeData
    );
    if (patientTypeData) {
      const totalPatients =
      ...
      if (totalPatients > 0) {
        ...
        console.log(
          "DepartmentsTab: Calculated returning rate from real data:",
          returningRate
        );
      } else {
        returningRate = Math.round(selectedDepartment.recommendRate); // Default to recommend rate
        console.log(
          "DepartmentsTab: Using recommend rate as fallback for returning rate:",
          returningRate
        );
      }
    } else {
      ...
      console.log(
        "DepartmentsTab: Using recommend rate as fallback for returning rate (no data):",
        returningRate
      );
    }
