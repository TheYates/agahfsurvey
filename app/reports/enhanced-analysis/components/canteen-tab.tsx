...
export function CanteenTab({ isLoading, departments }: CanteenTabProps) {
  ...
  useEffect(() => {
    if (departments && departments.length > 0) {
      console.log("CanteenTab: All departments:", departments);
      ...
      console.log("CanteenTab: Found canteen?", canteen);

      if (canteen) {
        console.log("CanteenTab: Using real canteen data:", canteen);
        setCanteenData({
          id: canteen.id,
          name: canteen.name,
        ...
        console.log(
          "CanteenTab: Checking for canteen ratings across departments"
        );
        ...
        departments.forEach((dept) => {
          if (dept.ratings && dept.ratings["food-quality"] !== undefined) {
            console.log(
              "CanteenTab: Found department with food-quality rating:",
              dept.name
            );
        ...
        if (hasCanteenRatings && totalVisits > 0) {
          ...
          console.log("CanteenTab: Created combined canteen data from ratings");
          setCanteenData({
            id: "canteen-combined",
          ...
          if (deptWithFoodRatings) {
            console.log(
              "CanteenTab: Using department with food ratings:",
              deptWithFoodRatings
            );
            setCanteenData({
            ...
            console.log("CanteenTab: Creating new canteen entry");
            // Use real survey data from concerns to at least show submissions
            fetchAllSurveyData()
              .then((surveyData) => {
                console.log(
                  "CanteenTab: Checking survey data for canteen submissions:",
                  surveyData
                );

                // Since there's no explicit canteen department, use all surveys as potential canteen data
                console.log(
                  "CanteenTab: Using all surveys as canteen data since no canteen department exists"
                );
                ...
                console.log(
                  `CanteenTab: Using ${visitCount} surveys for canteen data`
                );
  ...
  useEffect(() => {
    const loadCanteenConcerns = async () => {
      ...
      try {
        console.log("CanteenTab: Fetching concerns and surveys...");
        ...
        console.log("CanteenTab: Fetched all concerns:", allConcerns);
        console.log(
          "CanteenTab: Fetched all surveys for feedback:",
          allSurveys
        );
        ...
        console.log(
          "CanteenTab: Filtered canteen concerns:",
          canteenSpecificConcerns
        );
        ...
        if (canteenSpecificConcerns.length === 0 && allSurveys.length > 0) {
          console.log(
            "CanteenTab: Using general survey recommendations as concerns"
          );
          ...
          console.log(
            "CanteenTab: Created concerns from recommendations:",
            concernsFromRecommendations
          );
          ...
          console.log(
            "CanteenTab: No canteen concerns or recommendations found"
          );
          setCanteenConcerns([]);
        }
      } catch (error) {
